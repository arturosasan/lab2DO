...
public class EditorPredictivo extends ABB<String> {
    ...
    public ListaConPI<String> recuperarSucesores(String prefijo, int n) {       
        ...
        if (sucesor(prefijo) != null) { // está en la lista
            res.insertar(prefijo);
            n--;
        }
        String palabraABuscar = prefijo;
        while (palabraABuscar != null && n > 0) {
            palabraABuscar = super.sucesor(palabraABuscar); // actualizo la palabra que busco
            if (palabraABuscar != null && palabraABuscar.startsWith(prefijo)) { // hemos encontrado una palabra sucesora
                res.insertar(palabraABuscar);
                n--;
            }
        }
        return res;
