...
/** Clase TestEditorPredictivo: 
 *  programa grafico que hace uso del EditorPredictivo 
 *  para la redaccion de mensajes de texto.
 *  
 *  @author  Profesores EDA
 *  @version Febrero 2026
 */

public class GUIEditorPredictivoPlus extends JFrame implements 
    ...
    private static final String DICCIONARIO_POR_DEFECTO = "aplicaciones" 
        + java.io.File.separator + "editor" 
        + java.io.File.separator + "castellanoPlus.txt"; 
    private static final String ANYADIR_PALABRA = "\u00BFA\u00F1adir palabra?";
    private static final int MAX_SUCESORES = 40; // 20;
    
    // Atributos para el Editor Predictivo
    private String palabraActual;
    private EditorPredictivoPlus editor;
    // Componentes graficos
    private Font fSmall, fBig, fMed;
    private JButton btnClose, btnLoad, btnSave, btnDel, btnMode;
    private JLabel lblTitle, lblAlfabet, lblFreq;
    ...
    /** alfabetico = true  --> listado sugerencia por orden alfabético */
    /** alfabetico = false --> listado sugerencia por orden de frecuencias */
    private boolean alfabetico; 
	
	private ListaConPI<String> obtenerSugerencias() {
        ListaConPI<String> suc = new LEGListaConPI<>();
        if (alfabetico) {
            suc = editor.recuperarSucesoresAlfa(palabraActual, MAX_SUCESORES - 1);
        } else {
            ColaPrioridad<NodoPalabra> cp = editor.recuperarSucesoresFrec(palabraActual, MAX_SUCESORES - 1);
            LEGListaConPI<String> temp = new LEGListaConPI<String>();
            while (!cp.esVacia()) {
                temp.insertar(cp.eliminarMin().palabra);
            }
            for (temp.inicio(); !temp.esFin(); temp.siguiente()) {
                suc.inicio();
                suc.insertar(temp.recuperar());
            }
        }
        return suc;
    }
     ...
     */
    private GUIEditorPredictivoPlus() {
        super("EDITOR PREDICTIVO PLUS");
        editor = new EditorPredictivoPlus(DICCIONARIO_POR_DEFECTO);
        palabraActual = "";
        setUndecorated(true);
    ...
    private void inicializarInterfaz() {
        ...
        btnClose.setBounds(575, 5, 20, 20); 
        ...
        scrPane.setBounds(10, 40, 580, 260);     
        ...
        listScroller.setBounds(10, 320, 580, 80); 
        ...
        btnMode = new JButton("cambiar modo sugerencias");
        btnMode.setFont(fMed);
        btnMode.setBounds(400, 415, 188, 20);
        btnMode.setFocusPainted(false);
        btnMode.setMargin(new Insets(0, 0, 0, 0));
        btnMode.addActionListener(this);
        getContentPane().add(btnMode);
        
        lblAlfabet = new JLabel("ALFABÉTICO");
        lblAlfabet.setFont(new Font("arial", Font.BOLD + Font.ITALIC, 14));
        lblAlfabet.setForeground(setColorLabel(alfabetico));        
        lblAlfabet.setBounds(450, 435, 188, 20);
        getContentPane().add(lblAlfabet);
        
        lblFreq = new JLabel("FRECUENCIAS");
        lblFreq.setFont(new Font("arial", Font.BOLD + Font.ITALIC, 14));
        lblFreq.setForeground(setColorLabel(!alfabetico));
        lblFreq.setBounds(450, 455, 188, 20);
        getContentPane().add(lblFreq);
    }
    
    private Color setColorLabel(boolean b) {
        ...
        if (e.getSource() == btnLoad) {
            ...
            if (fileChooser.showOpenDialog(this) 
                ...
                if (!ext.toLowerCase().equals("txt")) {
                    ...
                    editor = new EditorPredictivoPlus(fichero.toString());
                    JOptionPane.showMessageDialog(this, 
     ...
     *  @param e Evento del raton
     */
    public void mouseReleased(MouseEvent e) { moviendo = false; }
    ...
    public static void main(String[] args) {
        GUIEditorPredictivoPlus app = new GUIEditorPredictivoPlus();
    ...
    private class JButtonRound extends JButton {
         ...
         *  @param posY      Posicion vertical del boton
         */
            ...
            if (activo) {
                ...
                if (s.length() > 0) {
                    ...
                    if (c == ' ' || c == ',' || c == '.') { 
                        ...
                        palabraActual += c;
                        ListaConPI<String> suc = obtenerSugerencias();                         
