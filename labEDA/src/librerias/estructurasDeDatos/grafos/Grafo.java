...
public abstract class Grafo {
    ...
    public Arista[] arbolRecubrimientoBFS() {
        Arista[] res = new Arista[numVertices() - 1]; // Es un árbol, no un grafo, queremos las aristas
        visitados = new int[numVertices()];
        ordenVisita = 0;
        ...
        while (!q.esVacia()) {
            int vActual = q.desencolar();
            ListaConPI<Adyacente> vAdyacentes = adyacentesDe(vActual);
            for (vAdyacentes.inicio(); !vAdyacentes.esFin(); vAdyacentes.siguiente()) {
                Adyacente aux = vAdyacentes.recuperar();
                if (visitados[aux.destino] == 0) {
                    res[numAristasArbol++] = new Arista(vActual, aux.destino, aux.peso);
                    visitados[aux.destino] = 1;
                    q.encolar(aux.destino);
                }
            }
        }
        return (numAristasArbol == numVertices() - 1) ? res : null; // Devuelvo null si el grafo NO es Conexo
    }
    ...
    public Arista[] kruskal() {       
        ...
        ColaPrioridad aristasFactibles = new MonticuloBinario(numAristas());
        UFSet uf = new ForestUFSet(numVertices());
        
        for(int i = 0; i < numVertices(); i++) {
            ListaConPI<Adyacente> adyacentes = adyacentesDe(i);
            if (adyacentes.esVacia()) return null;
            for(adyacentes.inicio(); !adyacentes.esFin(); adyacentes.siguiente()) {
                aristasFactibles.insertar((Comparable) new Arista(i, adyacentes.recuperar().destino, adyacentes.recuperar().peso));
            }
        }
