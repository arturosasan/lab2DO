...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    protected final void rehashing() {
        ...
        ListaConPI<EntradaHash<C,V>>[] aux = elArray; // COPIA SIN DESTRUIRLO
        elArray = new LEGListaConPI[newSize]; // LO ENLAZO?

        for(int i = 0; i < elArray.length; i++) { // INICIALIZAR NUEVO ARRAY
            elArray[i] = new LEGListaConPI<EntradaHash<C,V>>();
        }


        for(int i = 0; i < aux.length; i++) { // COPIA DE CUBETAS
            ListaConPI<EntradaHash<C,V>> lista = aux[i]; // EN AUX ESTÁN TODAS LAS LISTAS DE CADA CUBETA DEL ARRAY

            // POR CADA CUBETA HAY QUE SUMAR ** EL Nº DE TÉRMINOS QUE HAY EN CADA CUBETA

            lista.inicio();
            while(!lista.esFin()) { // RECORRIDO CUBETAS CON WHILE
                EntradaHash<C,V> entrada = lista.recuperar();
                elArray[indiceHash(entrada.clave)].insertar(entrada);
                lista.siguiente();
            }
            /*
            for(lista.inicio(); !lista.esFin(); lista.siguiente()) { // RECORRIDO CUBETAS CON FOR
                EntradaHash<C,V> entrada = lista.recuperar();
                elArray[indiceHash(entrada.clave)].insertar(entrada);
            }
             */
    ...
    public final double desviacionTipica() {
        double desvTip = 0;
        for (int i = 0; i < elArray.length; i++) {
             //desvTip = Math.pow((elArray[i].talla() - factorCarga()), 2)/ elArray.length;
            desvTip += (elArray[i].talla() - factorCarga()) *
                        (elArray[i].talla() - factorCarga()); // PARA NO HACER Math.pow(..., 2)
        }
        return Math.sqrt(desvTip /elArray.length); // Si haces aquí la division te ahorras muchos costes
    ...
    public final double costeMLocalizar() {
        ...
        for (int n = 0; n < elArray.length; n++) {
            colisiones += elArray[n].talla() * (elArray[n].talla() - 1) / 2;
        }
