...
public class ChatRobot {
  ...
  public static void main (String[] args) {
	...
	if (conf.getNick() == null) nick = "Botillo_asarsan"; else nick = conf.getNick();  //default nick
    if ((myChannelName=conf.getChannelName()) == null) myChannelName = "Par"; //default channel
    ...
    try {   
		...
		Queue auxQueue = (Queue) ctx.createTemporaryQueue();

	    //2.d COMPLETE: Assign the "replyTo" property to this temporary queue
	    
		msg.setJMSReplyTo(auxQueue);
		...
		try {
			...
			ctx.close();
			throw new Exception("ERROR, MESSAGE NOT SENT TO THE CHATSERVERJMS QUEUE");
		} 

        //2.f COMPLETE: Create a consumer for our temporary queue

		consumer = ctx.createConsumer(auxQueue);
		...
		msgJoin.setJMSReplyTo (auxQueue);
		...
		try {
			producer.send(channelTopic, ctx.createObjectMessage ( MessageFactory.chatMessage(nick," BIENVENIDO")));
		} catch (Exception e) {
			ctx.close();
			throw new Exception("CANNOT SEND MESSAGE TO CHATSERVERJMS QUEUE");
		}	
